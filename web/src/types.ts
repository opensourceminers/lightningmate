...
export interface BackupHealthStatus {
  ...
  channelChangesSinceLastBackup?: { opened: number; closed: number; pending: number };
...
export interface ChannelRiskStatus {
  activeChannelCount: number;
  inactiveChannelCount: number;
...
export interface OnchainSafetyStatus {
...
export interface LiquiditySafetyStatus {
  inboundSat: number;
  outboundSat: number;
  inboundRatio: number;
  ...
  maxReceiveEstimateSat: number;
  maxSendEstimateSat: number;
  concentration: { largestLocalChannelShare: number; largestRemoteChannelShare: number };
